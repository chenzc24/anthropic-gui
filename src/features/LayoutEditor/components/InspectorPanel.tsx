...
export const InspectorPanel: React.FC = () => {
  const { graph, selectedId, updateInstance, updateRingConfig } = useIORingStore();
        ...
        {selectedInstance ? (
          <div>
            <div className="mb-4 pb-2 border-b">
              <span className="text-xs bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full">Instance</span>
              <h2 className="text-lg font-bold mt-1 text-gray-800">{selectedInstance.name}</h2>
            </div>
            
            <PropertyEditor 
              data={selectedInstance} 
              onChange={(newData: any) => updateInstance(selectedInstance.id, newData)}
              readOnlyKeys={[]}
            />
          </div>
        ) : (
          <div>
            <div className="mb-4 pb-2 border-b">
              <span className="text-xs bg-green-100 text-green-800 px-2 py-0.5 rounded-full">Global</span>
              <h2 className="text-lg font-bold mt-1 text-gray-800">Ring Config</h2>
            </div>
